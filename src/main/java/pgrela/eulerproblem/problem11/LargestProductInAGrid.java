...
public class LargestProductInAGrid implements EulerSolver {
    public static final int[] THE_GRID = {8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8,
    ...
    public static void main(String[] args) {
        printSolution(LargestProductInAGrid.class);
    }

    public long solve() {
        Predicate<Point> inField = p -> p.x < 20 && p.y < 20 && p.x >= 0 && p.y >= 0;
        Function<Point, Integer> getValueByPoint = p -> inField.test(p) ? THE_GRID[p.x * 20 + p.y] : 0;

        return Stream.<BiFunction<Point, Integer, Point>>of(
                (p, i) -> point(p.x, p.y + i),
