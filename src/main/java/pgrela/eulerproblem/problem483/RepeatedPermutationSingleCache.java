...
public class RepeatedPermutationSingleCache implements EulerSolver {
    public static final int MAX_ALLOWED_LENGTH=400;
    public static final int DEFAULT_LENGTH = 50;

    public static void main(String[] args) {
        printSolution(RepeatedPermutationSingleCache.class);
    }
    ...
    private double calculateWithMaximalCycle(int length, int maximalCycleLength, LowestCommonMultiple lcm) {++c;
        ...
        if (maxLCMs[length][maximalCycleLength] != null && maxLCMs[length][maximalCycleLength].isDominatedBy(lcm)) {
            if (cache[length][maximalCycleLength] > -0.5) {
                return cache[length][maximalCycleLength] * pow(lcm.toDouble() /maxLCMsValues[length][maximalCycleLength], 2);
            }
        }
        ...
        LowestCommonMultiple lcmWithMCL = lcm.lcmWith(maximalCycleLength);
    ...
    public RepeatedPermutationSingleCache() {
        this(DEFAULT_LENGTH);
    }

    public RepeatedPermutationSingleCache(int length) {
        this.length = length;
    ...
    private LowestCommonMultiple getLCM(int cachedLength, int maximalCycle) {
        ...
        for (int i = 2; i <= maximalCycle; i++) {
            lcm = lcm.lcmWith(LowestCommonMultiple.toPoweredLCM(i, 1));
        }
